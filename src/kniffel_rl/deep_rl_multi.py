...
class KniffelRL:
    ...
    def _train(
        self, nb_steps=10_000, load_path="", env_config="", logging=False, file_name=""
    ):
        env = KniffelEnv(
            env_config,
        ...
        path = f"{self._path_prefix}output/weights/{file_name}"
        ...
        json_object = json.dumps(self._hyperparater_base, indent=4)
...
def train(rl: KniffelRL, env_config: dict, file_name: str):
    """Train a model
...
if __name__ == "__main__":
    ...
    hps_path = str(Path(__file__).parents[2]) + "/src/config/hyperparameters.json"
